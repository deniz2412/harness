...
| **F11** | **No egress control.** Compose declares no networks; every container gets the default bridge with full outbound internet. Spec §2.3's "egress limited to gateway + GitHub" is aspirational — nothing implements it. | **Medium** | Not urgent while no tool takes a URL. It becomes the difference between a sandbox and a costume the moment `bash.sandboxed` exists. Internal network for postgres now; a real egress policy with the runner work. | **M2 — blocks the runner** |
