...
**Date:** 25 Jul 2026 · **Status:** v2.14 — local MVP scope; **M0–M3 ✅, F1 ✅, M5 ✅, M6 ✅, M7 ✅, M7b ✅,
M7c ✅, F2 ✅, F3 ✅, F4 ✅ complete — the PoC-buildable roadmap (M0–M7c + F1–F4) is done** (exit checks in
`docs/m0-exit-check.md` … `docs/f1-exit-check.md`, `docs/m5-exit-check.md`, `docs/m6-exit-check.md`, `docs/m7-exit-check.md`,
`docs/m7b-exit-check.md`, `docs/m7c-exit-check.md`, `docs/f2-exit-check.md`, `docs/f3-exit-check.md`, `docs/f4-exit-check.md`; reviews in `REVIEW.md`). Highlights: M2's write path
...
workflow catalog & launcher on the F1 console (browse, DAG detail, launch, per-workflow stats); F3 an
authoring workbench (validate editor YAML against the floor/catalog + dry-run DAG + publish-preview,
never executing or writing); F4 suite dashboards + a YAML-emitting drag-and-drop visual builder (the
last frontend increment). **The PoC-buildable roadmap is complete; M4/M8 graduation to real infra
(OpenShift, Vault, SIEM, SSO) is the deferred next step, on the human's decision.**
